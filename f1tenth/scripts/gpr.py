...
def risk(x,y,cx,cy):
    ...
    risk_ = 20*mean_dist + 1*max_dist+10*final
...
def generate_exp_data(N,visualize=False):
    ...
    for i in range(N):
        ...
        file_exp.close()
        file_demo.close()
        risk_exp,mean_exp,max_exp,final_exp = risk(np.array(data_exp['states']['x'])+1.48,np.array(data_exp['states']['y'])+3.8,data_demo['X'],data_demo['Y'])
...
def generate_test_data(N):
    ...
    risk_np = np.zeros((1,1))
    for i in range(N):
        file_demo = open('/home/anjali/learn_from_sparse/f1tenth/data/gpr/experiments/test_demo/test_initial_'+str(i)+'.pkl','rb')
        data_demo = pickle.load(file_demo)
        file_exp = open('/home/anjali/learn_from_sparse/f1tenth/data/gpr/experiments/test_experiments/data_test_initial_'+str(i)+'.pkl','rb')
        data_exp = pickle.load(file_exp)
        file_exp.close()
        file_demo.close()
        #breakpoint()
        risk_exp = risk(data_exp['states']['x'],data_exp['states']['y'],np.array(data_demo['X'])-1.48,np.array(data_demo['Y'])-3.8)
        #risk_sim = risk(data_demo['X_sim'],data_demo['Y_sim'],data_demo['X'],data_demo['Y'])
        print('Risk_exp:',risk_exp[0],'ID:',i)
        
        Z_exp = np.array([data_demo['lw'],data_demo['V_ref']])
        Z_np = np.vstack((Z_np,Z_exp)) 
        risk_np = np.vstack((risk_np,risk_exp[0]))   
    Z_np = Z_np[1:,:]
    risk_np =  risk_np[1:,:]
    return Z_np, risk_np
...
def data_collect(N,N_Sim,visualize=True):
    ...
    if visualize==True:
        ...
        plt.savefig('GPR_experiments.png')
        plt.show()
...
def eval(x_test,y_test,model,likelihood):
    ...
    plt.axhline(0.5,0,10,linestyle='--',linewidth=2,color='k',label='$R_{th}$')
    ...
    plt.savefig('Random_Test_Error.png')
    plt.show()
...
def visualize_gpr(Z_initial,y_test,model,likelihood):#model1,likelihood1,model2,likelihood2,model3,likelihood3):
    ...
    #file_i = open("/home/anjali/learn_from_sparse/f1tenth/data/gpr/experiments/initial_"+str(N_shape)+".pkl",'wb')
    ...
    plt.show()
    plt.savefig('GPR_scattering.png')
    #breakpoint()risk_init

    N = Z_initial.shape[0]
    plt.figure()
    #plt.contourf((X*4.0)+0.5,Y*6.5,mean_preds.T,levels=10)
    plt.contourf(X*4+0.5,Y*6+0.5,mean_preds.T,levels=10)
    ...
    plt.savefig('GPR_Risk'+str(30)+'.png')

...
#Set center and radius according to the mean and radius of the Gaussian distribution to sample from
def h(Z):
    return np.power(np.linalg.norm(Z-np.array([2,0])),2) - 4
...
def projection_circle(Z,h,k,r):
    x1 = Z[0]
    x2 = Z[1]
    dist = np.linalg.norm(np.array([x1-h,x2-k]))
    if dist>r:
        Z_proj = np.array([h+((r*(x1-h)))/dist,k+((r*(x2-k))/dist)])  
    else:
        Z_proj = Z
    return Z_proj#np.array([h+(r*(x1-h))/dist,k+(r*(x2-k))/dist])  
...
def mcmc_mh(Z_curr,prob_curr,func, proposal_func, proposal_func_prob): 
    ...
    Z_new = projection_circle(Z_int,h=2,k=0,r=2.5)

    #prob_curr = func(Z_curr) #Calculate likelihood of Z_t
    prob_new,risk_new, mean_risk,max_risk,final_risk = func(Z_new) #Calculate likelihood of Z^*
    ...
    if acceptance_prob > st.uniform(0,1).rvs():
        ...
        Z_t_plus = Z_curr
        accept = 0
    return Z_t_plus, accept,prob_new,risk_new,mean_risk,max_risk,final_risk 
